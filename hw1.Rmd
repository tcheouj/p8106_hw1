...
The lambda value, our tuning parameter, that minimizes MSE is given by `r lasso_glmnet_cv$lambda.min`, and the smallest lambda value within 1 SE of the MSE is given by `r lasso_glmnet_cv$lambda.1se`. 
...
For the optimal lambda values, both the `glmnet` (`r lasso_glmnet_cv$lambda.min`) and `caret` (`r lasso_caret$bestTune$lambda`) implementations get the same numbers. However, the lambdas within the 1 SE rule are also the different (`r lasso_glmnet_cv$lambda.1se` for `glmnet` and `r lasso_caret_1se$bestTune$lambda` for `caret`). The differences in this may be in the implementation of getting the 1 SE of the regularization parameter, since `caret` requires fitting a new model altogether using the prespecified control parameters with the 1 SE rule, while the lambda within 1 SE is a subobject of the model outputted by `glmnet`. 

```{r}
lasso_glmnet_cv_pred <- predict(lasso_glmnet_cv, s = lasso_glmnet_cv$lambda.min, newx = test, type = "response")

lasso_glmnet_cv_testerror <- mean((lasso_glmnet_cv_pred - testing[, "Sale_Price"])^2) 
```

After fitting the lasso regression model to the testing dataset, the test error is `r lasso_glmnet_cv_testerror`.
